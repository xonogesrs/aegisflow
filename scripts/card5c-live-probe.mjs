// scripts/card5c-live-probe.mjs
// V2 Card 5C Stage 3–8 — refrozen small live probe（E2 → E6 → E9 → E4，各一次）。
//
// 固定條件：provider requests=1、retry=0、repair=0、resample=0、fallback=0、tools 缺席、
// session reuse=0（每案全新 adapter）、prompt mutation after start 禁止。
// 舊 Card 5 E2 request 不計入本 gate。
...
import { buildPromptBundle, buildUserPrompt } from "../src/v2/prompt-builder.mjs";
...
import { EVIDENCE_ROOT } from "./card5c-freeze-manifest.mjs";

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const sha = (s) => createHash("sha256").update(s).digest("hex");
const FROZEN_CONFIG_HASH = sha(JSON.stringify(TRANSPORT_FREEZE));

// 凍結 manifest：取最新 card-5c-freeze-manifest-*（不覆寫）
function latestFreezeManifest() {
  const dirs = readdirSync(EVIDENCE_ROOT).filter((d) => d.startsWith("card-5c-freeze-manifest-")).sort();
  if (dirs.length === 0) throw new Error("no card-5c freeze manifest found");
  return { id: dirs[dirs.length - 1], path: `${EVIDENCE_ROOT}/${dirs[dirs.length - 1]}/freeze-manifest.json` };
}

// Stage 8 failure classification（最精確 failure code）
function classifyFailure(pipeline, transport, run) {
  ...
  switch (pipeline.stage) {
    ...
    case "case": {
      ...
      if (codes.includes("PROHIBITED_EFFECT_PRESENT")) return "PROHIBITED_EFFECT_PRESENT";
      if (codes.includes("INVALID_COVERAGE_CLAIM")) return "INVALID_COVERAGE_CLAIM";
      return "SCORECARD_HARD_FAILURE";
    }
    case "scorecard":
      return pipeline.verdict === "NOT_BENEFICIAL" ? "UNEXPECTED_NOT_BENEFICIAL" : "SCORECARD_HARD_FAILURE";
    default:
      return "SCORECARD_HARD_FAILURE";
  }
}

// tee fetch：capture request body hash、HTTP status、redirects、response model、response id、raw finish_reason
...
function singleCaseCheck(ev) {
  ...
  if (ev.redirect_count !== 0) failures.push(`redirect contract violation: ${ev.redirect_count} redirects`);
...
async function main() {
  ...
  const dir = `${EVIDENCE_ROOT}/card-5c-live-probe-${new Date().toISOString().replace(/[:.]/g, "-")}`;
  ...
  for (const caseId of PROBE_ORDER) {
    ...
    // freeze 驗證：prompt hash 必須與 5C manifest 一致（證明未改動、無洩漏）
    ...
    const failureCode = failures.length ? classifyFailure(pipeline, transport, run) : null;
  ...
  const summary = {
    ...
    verdict: hardStop.triggered
      ? `HOLD / ${hardStop.code}`
      : "PASS / V2_CARD_5C_REFROZEN_SMALL_LIVE_PROBE_COMPLETE",
  };
  ...
  writeFileSync(`${dir}/report.md`, `# V2 Card 5C — Refrozen Small Live Probe\n\n${JSON.stringify({ summary }, null, 2)}\n`);

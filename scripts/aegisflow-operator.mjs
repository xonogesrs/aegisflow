#!/usr/bin/env node
// scripts/aegisflow-operator.mjs
//
...
//
//   node scripts/aegisflow-operator.mjs --run <graphRunId> [--json] [--root <dir>]
//
...
//                        (AEGISFLOW_TELEMETRY_STATE_ROOT as consumed by
...
//                        used.
//
// The pre-rename file name (scripts/autoloop-operator.mjs) still works: it is
// a forwarding alias, so existing automation does not have to be repointed.
//
...
const usage = () => {
  console.error("usage:");
  console.error("  node scripts/aegisflow-operator.mjs --run <graphRunId> [--json] [--root <dir>]");
  process.exit(2);
};
...
const env = rootOverride
  ? { ...process.env, AEGISFLOW_TELEMETRY_STATE_ROOT: resolve(rootOverride) }
  : process.env;
